...
def point_on_polygon_boundary(
    point: np.ndarray, polygon: np.ndarray, tol: float = 1e-10
) -> bool:
...
def point_on_line_segment(
    point: np.ndarray, p1: np.ndarray, p2: np.ndarray, tol: float = 1e-10
) -> bool:
    ...
    if np.dot(v, v) < tol:  # p1 and p2 are the same point
        return bool(np.linalg.norm(point - p1) < tol)
...
def segments_intersect(
    p1: np.ndarray, p2: np.ndarray, p3: np.ndarray, p4: np.ndarray, tol: float = 1e-10
) -> bool:
...
def validate_edge_embedding(
    edge_coords: np.ndarray,
    all_coords: np.ndarray,
    edge_vertex_indices: List[int],
    all_vertex_indices: List[int],
    tol: float = 1e-10,
) -> Tuple[bool, str]:
    """
    ...
    for i, vertex_idx in enumerate(all_vertex_indices):
        ...
        if point_on_line_segment(vertex_coord, p1, p2, tol):
            ...
            if (
                np.linalg.norm(vertex_coord - p1) > tol
                and np.linalg.norm(vertex_coord - p2) > tol
            ):
