...
class ECT:
    ...
    def __init__(
        ...
        self.num_dirs = len(directions) if directions is not None else num_dirs
        self.num_thresh = num_thresh or num_dirs
    ...
    def get_thresholds(
        ...
        if override_radius is not None:
            if override_radius <= 0:
                raise ValueError(
                    f"Bounding radius must be positive, got {override_radius}"
                )
            return override_radius, np.linspace(
                -override_radius, override_radius, self.num_thresh
            )
    ...
    def calculate(
        self, graph: Union[EmbeddedGraph, EmbeddedCW], theta=None, bound_radius=None
    ):
