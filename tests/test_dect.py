...
class TestDECTBasicFunctionality(unittest.TestCase):
    ...
    def test_inheritance_from_ect(self):
        ...
        # Verify only _compute_directional_transform is different
        ect = ECT(
            num_dirs=self.num_dirs,
            num_thresh=self.num_thresh,
            bound_radius=self.bound_radius,
        ...
        # These methods should be the same (inherited)
        self.assertEqual(
            dect._ensure_directions.__code__.co_code,
            ect._ensure_directions.__code__.co_code
        )
        self.assertEqual(
            dect._ensure_thresholds.__code__.co_code,
            ect._ensure_thresholds.__code__.co_code
        )
        
        # This method should be different (overridden)
        self.assertNotEqual(
            dect._compute_directional_transform.__code__.co_code,
            ect._compute_directional_transform.__code__.co_code
        )
