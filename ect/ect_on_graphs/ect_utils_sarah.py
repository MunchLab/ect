...
def ECC(G, pos, theta, r, numThresh):
    ...
    for i in range(numThresh):
        ...
        if i == numThresh-1:
            newV = list(
                compress(v_list, [r_threshes[i] > g(v) for v in v_list]))
        else:
            newV = list(compress(v_list, [r_threshes[i] > g(
                v) >= r_threshes[i+1] for v in v_list]))

        # previous value of ECC (not index i-1 becuase of extra 0 appended to begining)
        x = ecc[i]
